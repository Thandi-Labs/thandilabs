      ...
      <div className="border-b border-zinc-100 bg-white px-6 py-5 dark:border-white/5 dark:bg-zinc-900">
        <div className="mx-auto flex max-w-5xl items-center justify-between">
          <div>
            ...
            <p className="mt-0.5 text-sm text-zinc-400 dark:text-zinc-500">{getDate()}</p>
          </div>
      ...
      <div className="mx-auto max-w-5xl px-6 py-8 space-y-10">
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          {STATS.map(({ label, value, sub, icon: Icon, color, bg }) => (
            ...
            >
              ...
              <p className="mt-2 text-[11px] text-zinc-400 dark:text-zinc-600">{sub}</p>
            </div>
          ))}
        </div>
        ...
        <section>
          <div className="mb-4 flex items-center justify-between">
            <div>
              <h2 className="font-semibold text-zinc-900 dark:text-white">Your services</h2>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            {ACTIVE_SERVICES.map(({ icon: Icon, label, tag, accent, metrics, manageHref }) => (
              <div
                key={label}
              ...
              >
                ...
                <div className="flex items-start justify-between px-5">
                  <div className="flex items-center gap-3">
                    <div className={`flex h-10 w-10 items-center justify-center rounded-lg ${accentIcon[accent]}`}>
                      <Icon className="h-5 w-5" />
                    </div>
                    <div>
                      <h3 className="font-semibold text-zinc-900 dark:text-white">{label}</h3>
                      <span className={`mt-0.5 inline-block rounded-full px-2 py-px text-[10px] font-medium ${accentTag[accent]}`}>
                        {tag}
                      </span>
                    </div>
                  </div>
                ...
                <div className="mt-5 grid grid-cols-2 divide-x divide-y divide-zinc-100 border-t border-zinc-100 dark:divide-white/5 dark:border-white/5">
                  {metrics.map(({ label: ml, value }) => (
                    <div key={ml} className="px-5 py-3">
                      ...
                      <p className="text-xs text-zinc-400 dark:text-zinc-500">{ml}</p>
                    </div>
                  ))}
                </div>
        ...
        <section id="expand">
          <div className="mb-4">
            <h2 className="font-semibold text-zinc-900 dark:text-white">Expand your stack</h2>
            <p className="text-sm text-zinc-400 dark:text-zinc-500">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {AVAILABLE_SERVICES.map(({ icon: Icon, label, tag, accent, description, price }) => (
              <div
                key={label}
              ...
              >
                <div className="flex items-start justify-between">
                  ...
                  <span className={`rounded-full px-2.5 py-0.5 text-[10px] font-medium ${accentTag[accent]}`}>
                    {tag}
                  </span>
                </div>

                <h3 className="mt-4 font-semibold text-zinc-900 dark:text-white">{label}</h3>
                <p className="mt-1 text-sm leading-relaxed text-zinc-500 dark:text-zinc-400">
                  {description}
                </p>

                <div className="mt-5 flex items-center justify-between">
                  <p className="text-sm font-medium text-zinc-700 dark:text-zinc-300">{price}</p>
                  <button className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-medium transition-all duration-200 border ${
                    accent === "amber"
